...
First, let's start with the fixed effects estimator which is estimated by demeaning each equation.

To calculate the means let: $\overline{y}_i = \dfrac{{y}_i1 + {y}_i2}{2}$, 
$\overline{x}_i = \dfrac{{x}_i1 + {x}_i2}{2}$, 
...
Substituting in the means: \[\sum_{t=1}^{2}[x_{it}-\overline{x}_i]'[x_{it}-\overline{x}_i] = \sum_{t=1}^{2}[x_{it}-\dfrac{{x}_{i1} + {x}_{i2}}{2}]'[x_{it}-\dfrac{{x}_{i1} + {x}_{i2}}{2}] = \]


\[[\dfrac{{x}_{i1} - {x}_{i2}}{2}]'[\dfrac{{x}_{i1} - {x}_{i2}}{2}]+[\dfrac{{x}_{i2} - {x}_{i1}}{2}]'[\dfrac{{x}_{i2} - {x}_{i1}}{2}] = \]

\[[\dfrac{\Delta x_{i}'\Delta x_{i}}{2}] \]

Similarly for the second half: \[\sum_{t=1}^{2}[x_{it}-\overline{x}_i]'[y_{it}-\overline{y}_i] = \sum_{t=1}^{2}[x_{it}-\dfrac{{x}_{i1} + {x}_{i2}}{2}]'[y_{it}-\dfrac{{y}_{i1} + {y}_{i2}}{2}] = \]


\[[\dfrac{{x}_{i1} - {x}_{i2}}{2}]'[\dfrac{{y}_{i1} - {y}_{i2}}{2}]+[\dfrac{{x}_{i2} - {x}_{i1}}{2}]'[\dfrac{{y}_{i2} - {y}_{i1}}{2}] = \]

\[[\dfrac{\Delta x_{i}'\Delta y_{i}}{2}] \]
...
And the 2s cancel each other out.
...
\[y_{i2}-{y}_{i1} = [x_{i2}-{x}_{i1}]\beta + [\epsilon_{i2}-{\epsilon}] \rightarrow \Delta y_{i} = \Delta y_{i}\beta + \Delta \epsilon_{i}\]
...
The same resulst happens for t = 2. Then, back to \[\sigma^2_{FE}\]

Plug in what we just solved for to get \[\sigma^2_{FE} = \dfrac{1}{2}\sigma^2_{DE}\]

Then, we can show that the variances are the same to difference estimator:
...
One notices that this final thing is the Variance of the coefficient on the fixed effects.
