...
#1c - Summary Stats
```{r 1c}
sumstat <- as.data.frame(cbind(apply(dat,2,mean),
                               apply(dat,2,sd),
                               apply(dat,2,min),
                               apply(dat,2,max))) %>% 
...
psmoking.dat <- as.data.frame(cbind(omaps,fmaps,bweight)) %>% 
  set_colnames(c("One-Minute APGAR Score", "Five-Minute APGAR Score", "Birthweight")) %>% 
  set_rownames(c("Smoking While Pregnant", "No Smoking While Pregnant"))
```
...
```{r}
psmoke.all <- as.data.frame(cbind(Variables, round(colMeans(dat),3), 
                                  round(colMeans(dat[-which(dat$tobacco == 1),]),3),
                                  round(colMeans(dat[-which(dat$tobacco == 2),]),3))) %>% 
set_colnames(c("Variable","Avg Birthweight","Avg Birthweight (Non-Smoker while Pregnant)",
                 "Avg Birthweight (Smoker while Pregnant)"))
```
...
olsvars <- cbind(olsvars, as.character(psmoke.all$Variable[olsvars])) %>% as.data.frame() %>% set_colnames(c("Code", "Variable"))
...
lm.out <- lm(dbrwt ~ stresfip+dmage+ormoth+mrace3+dmeduc+dmar+adequacy+
               ...
               preterm+tobacco+cigar+alcohol+drink+wgain, data = dat)
summary(lm.out)
```
